...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	net.InitName(net, "num_emb")

	in := net.AddLayer2D("Input", 1, 82, emer.Input)
	hid := net.AddLayer2D("Hidden", 3,10, emer.Hidden)
	out := net.AddLayer2D("Output", 1, 82, emer.Target)
	...
	//net.LayerByName("Hidden1").(leabra.LeabraLayer).AsLeabra().LesionNeurons(0)
}
...
func (ss *Sim) TestTrial(returnOnChg bool) {
	...
	ss.TrialStats()
	print(ss.TestEnv.TrialName.Cur,",")

	for i := 0; i < 30; i++ {
		val := ss.Net.LayerByName("Hidden").UnitVal1D(0,i)
		if(val > 0.5) {
			print(1)
		} else {
			print(0)
...
func (ss *Sim) ConfigPats() {
	...
	dt.SetMetaData("desc", "Training patterns for operands and operators")
	sch := etable.Schema{
			{"Name", etensor.STRING, nil, nil},
			{"Input", etensor.FLOAT32, []int{1, 81}, nil},
			{"Output", etensor.FLOAT32, []int{1, 81}, nil}, // Adjusted for 7x1 output
	}
	dt.SetFromSchema(sch, 5) // Assume 25 training patterns for example
}
...
func (ss *Sim) OpenPats() {
	...
	// Assuming the file path to the new pattern file is correctly specified
	err := dt.OpenCSV("dataset.tsv", etable.Tab) // Ensure the file path and name are correct
	if err != nil {
			log.Println("Failed to load training patterns:", err)
	}
}
...
func (ss *Sim) ConfigGui() *gi.Window {
	title := "Numerical Embedding Network"
	ss.GUI.MakeWindow(ss, "num_emb", title, `This demonstrates a basic Leabra model. See <a href="https://github.com/emer/emergent">emergent on GitHub</a>.</p>`)
