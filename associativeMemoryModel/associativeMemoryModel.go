...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	net.InitName(net, "assoc_mem")

	in := net.AddLayer4D("Input", 3, 1, 3, 10, emer.Input)
	hid := net.AddLayer2D("Hidden", 30, 30, emer.Hidden)
	out := net.AddLayer2D("Output", 3, 10, emer.Target)
...
func (ss *Sim) ConfigPats() {
	...
	sch := etable.Schema{
			...
			{"Operand1", etensor.FLOAT32, []int{4, 1}, nil},
			{"Operand2", etensor.FLOAT32, []int{4, 1}, nil},
			{"Operator", etensor.FLOAT32, []int{4, 1}, nil},
			{"Output", etensor.FLOAT32, []int{7, 1}, nil}, // Adjusted for 7x1 output
	}
...
func (ss *Sim) OpenPats() {
	...
	err := dt.OpenCSV("data/full-dataset.tsv", etable.Tab) // Ensure the file path and name are correct
	if err != nil {
...
func (ss *Sim) ConfigGui() *gi.Window {
	...
	ss.GUI.MakeWindow(ss, "assoc_mem", title, `This demonstrates a basic Leabra model. See <a href="https://github.com/emer/emergent">emergent on GitHub</a>.</p>`)
